...
pub fn activate_hermes_ccr() -> Result<()> {
    ...
    activate_hermes_ccr_with_config(&config, &hermes_config_path())
}

fn activate_hermes_ccr_with_config(config: &Config, path: &Path) -> Result<()> {
    let port = ccr_port(config);
    let model = first_route_pool_client_model(config, DEFAULT_MODEL)?;
    install_hermes_ccr(path, port, &model)
}
...
"#;
    ...
    fn activate_fails_before_writing_when_route_pool_has_no_active_route() {
        ...
        let error = activate_hermes_ccr_with_config(&disabled_route_pool_config(), &path)
            .unwrap_err()
            .to_string();
